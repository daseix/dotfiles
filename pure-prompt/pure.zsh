...
prompt_pure_preprompt_render() {
	...
	if [[ -n $prompt_pure_git_arrows ]]; then
		preprompt_parts+=('%B%F{$prompt_pure_colors[git:arrow]}${prompt_pure_git_arrows}%f%b')
	fi
    ...
    local cleaned_ps1="$prompt_pure_state[username]%B${PROMPT}%f%b"
		...
		prompt_pure_async_git_dirty)
			...
			if (( code == 0 )); then
                ...
                if (( n_modified > 0)); then
                    prompt_pure_git_dirty="%b$prompt_pure_git_dirty ${n_modified}%F{red}M%f"
                fi
                if (( n_untracked > 0)); then
                    prompt_pure_git_dirty="%b$prompt_pure_git_dirty ${n_untracked}%F{white}?%f"
                fi
			fi
...
prompt_pure_state_setup() {
	...
	hostname='%F{$prompt_pure_colors[path]}@%F{$prompt_pure_colors[host]}%m%f'
