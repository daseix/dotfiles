...
prompt_pure_preprompt_render() {
	...
	#preprompt+=" %F{$STATUS_COLOR}%c%f%b"
	preprompt+=" %F{$STATUS_COLOR}%2~%f%b"

	# git info
	preprompt+="%F{$git_color}${vcs_info_msg_0_}${prompt_pure_git_dirty}%f"

	# git pull/push arrows
	preprompt+="%B%F{blue}${prompt_pure_git_arrows}%f%b"

	# execution time
	#preprompt+="%B%F{242}${prompt_pure_cmd_exec_time}%f%b"

	# symbol, colored by previous command exit code
	#preprompt+="%F{$symbol_color}${PURE_PROMPT_SYMBOL:-❯}%f "
	preprompt+="%B%f${PURE_PROMPT_SYMBOL:-⋗}%f%b"
...
prompt_pure_setup() {
    ...
    prompt_pure_username='%f%n%F{green}@%f%m%f'
	#prompt_pure_username='%F{$STATUS_COLOR}%n@%m%f'
